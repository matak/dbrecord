...
abstract class DbRecord extends FreezableObject implements \ArrayAccess, IObjectContainerToFree
{
	/** @var string table name */
	protected static $_table;

	/** @var string main index */
	protected static $_mainIndex;
	...
	/** @var string default configuration object */
	const DEFAULT_CONFIG = '\System\DbRecord\DbRecordConfig';
	...
	/** \System\DbRecord\DbRecord $parent */
	/*protected $parent;*/
	...
	protected function detectState(array $values)
	{
		...
		$primaryColumns = static::config()->getPrimaryColumns();
	...
	{
		$config = static::config();
		$columns = $config->getColumnsKeys();
		$data = array();
		foreach ($columns as $column) {
			if (array_key_exists($column, $this->_data) || array_key_exists($column, $this->_associations)) {
				$type = $config->getType($column);
				if ($type == 'd' || $type == 't') {
					$d = $this->$column;
					if (!$d) {
						$value = NULL;
	...
	public function save()
	{
		...
		elseif ($this->isRecordNew()) {
			static::mapper()->insert($this);
		}
		elseif ($this->isRecordExisting()) {
			static::mapper()->update($this);
		}
