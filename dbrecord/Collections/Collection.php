...
abstract class Collection extends \ArrayObject implements ICollection
{
	...
	public function import($arr)
	{
		if (!(is_array($arr) || $arr instanceof \Traversable)) {
			throw new \Nette\InvalidArgumentException("Argument must be traversable.");
		}
	...
	{
		throw new Nette\DeprecatedException(__METHOD__ . '() is deprecated; use freeze() instead.');
	}
	...
	{
		throw new Nette\DeprecatedException(__METHOD__ . '() is deprecated; use isFrozen() instead.');
	}
	 ...
	 * @param  mixed
	 * @return void
	 * @throws \Nette\InvalidArgumentException, \NotSupportedException
	 */
		...
		if ($this->itemType !== NULL) {
			if ($this->checkFunc === NULL) {
				if (!($item instanceof $this->itemType)) {
					throw new \Nette\InvalidArgumentException("Item must be '$this->itemType' object.");
				}
			} else {
				$fnc = $this->checkFunc;
				if (!$fnc($item)) {
					throw new \Nette\InvalidArgumentException("Item must be $this->itemType type.");
				}
			}
		}
	}
	...
	{
		throw new Nette\NotSupportedException('Use ' . __CLASS__ . '::import()');
	}
	...
	public static function __callStatic($name, $args)
	{
		...
		throw new Nette\MemberAccessException("Call to undefined static method $class::$name().");
	}
	...
	{
		throw new Nette\MemberAccessException("Cannot unset the property {$this->reflection->name}::\$$name.");
	}
	...
	protected function updating()
	{
		if ($this->frozen) {
			...
			throw new Nette\InvalidStateException("Cannot modify a frozen object '$class'.");
		}
	}
